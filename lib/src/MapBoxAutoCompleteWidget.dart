...
class _MapBoxAutoCompleteWidgetState extends State<MapBoxAutoCompleteWidget> {
  ...
  //          "https://api.mapbox.com/geocoding/v5/mapbox.places/$input.json?access_token=${widget.apiKey}&autocomplete=true&language=${widget.language}&limit=${widget.limit}&proximity=ip&fuzzyMatch=true&types=place%2Cpostcode%2Caddress%2Cdistrict%2Clocality%2Cneighborhood%2Cregion%2Cpoi";

  Predections? _placePredictions = Predections.empty();

  Future<void> _getPlaces(String input) async {
    if (input.length > 0) {
      String url = "https://api.mapbox.com/geocoding/v5/mapbox.places/$input.json?access_token=${widget.apiKey}&cachebuster=1566806258853&autocomplete=true&language=${widget.language}&limit=${widget.limit}";
    ...
    return Scaffold(
      appBar: AppBar(
        systemOverlayStyle: SystemUiOverlayStyle(
          systemNavigationBarColor: Color(0xff2E63F6), // Navigation bar
          statusBarColor: Color(0xff2E63F6), // Status bar
        ),
        backgroundColor: Color(0xff2E63F6),
